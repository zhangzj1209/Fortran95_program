...
- `Ew.d[Ee]` 用科学技术法，以w个字符长来输出实数，小数部分占d个字符长，指数部分最少输出e个数字
```fortran
...
- `Gw.d` 以w个字符长来输出所有类型的数据，d不一定会使用，但不能省略。用来输入/输出 字符/整数/逻辑数 时，`Gw.d=Aw/Iw/Lw`，d必须随便给一个数字，不能省略；用来输入/输出 实数 时，`Gw.d=Fw.d`或`Ew.d`
```fortran
write(*, "(G11.2)") 123.45
```
